...
OUTPUT_ROOT="${OUTPUT_ROOT:-results/forensiczip/fakeclue}"
...
echo "[ForensicZip/FakeClue] model=${MODEL_PATH_7B}"
echo "[ForensicZip/FakeClue] output=${OUTPUT_ROOT}"
echo "[ForensicZip/FakeClue] ratios=${RETENTION_RATIOS[*]}"
...
for ratio in "${RETENTION_RATIOS[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES="${CUDA_DEVICES}" "${PYTHON_BIN}" scripts/eval_forensiczip.py \
    --dataset_type fakeclue \
    --model_path "${MODEL_PATH_7B}" \
    --val_batch_size "${VAL_BATCH_SIZE}" \
    --workers "${WORKERS}" \
    --output_path "${out_dir}" \
    --test_json_file "${FAKECLUE_TEST_JSON}" \
    --data_base_test "${FAKECLUE_DATA_BASE}" \
    --exp_name "forensiczip_fakeclue_r${ratio}" \
    "${processor_args[@]}" \
